# stock_recommendation_backtest_v2.py
...
BACKTEST_OUTPUT_FILE = PROJECT_DIR / "stock_recommendation_backtest_result_v2.csv"
SUMMARY_OUTPUT_FILE = PROJECT_DIR / "stock_recommendation_backtest_summary_v2.csv"
        ...
        for grade_key in BACKTEST_GRADES:
            ...
            for item in picks:
                ...
                if isinstance(tech, dict):
                    row["signal_open"] = tech.get("open")
                    row["signal_high"] = tech.get("high")
                    row["signal_low"] = tech.get("low")
                    ...
                    row["signal_upper_shadow_ratio"] = tech.get("upper_shadow_ratio")
                    row["signal_body_ratio"] = tech.get("body_ratio")
...
def load_recommendations(file_path: Path = STOCK_RECOMMENDATION_FILE) -> pd.DataFrame | None:
    ...
    df = df.drop_duplicates(subset=["recommend_date", "stock_id", "grade"])
    ...
    print(df[["recommend_date", "stock_id", "stock_name", "grade"]].head(30).to_string(index=False))
...
def find_entry_open_next_day(recommendation_row: pd.Series, price_df: pd.DataFrame) -> dict | None:
    rec_dt = pd.to_datetime(recommendation_row["recommend_date"])
    ...
    return {
        ...
        "entry_reason": "推薦日隔一交易日開盤進場",
        "entry_wait_days": 1,
    }


def find_entry_pullback_ma(recommendation_row: pd.Series, price_df: pd.DataFrame) -> dict | None:
    """
    回測 5MA / 10MA 區間才進場。

    規則：
    - 推薦後最多等 MAX_ENTRY_WAIT_DAYS 個交易日
    - 使用推薦當下的 signal_ma5 / signal_ma10 作觀察區
    - 當日最低價 <= 觀察區上緣，且收盤不跌破觀察區下緣，視為回測不破
    - 進場價採 max(觀察區下緣, 當日最低價到觀察區內的合理價)
    - 為保守與簡化，實際進場價設為觀察區上緣
    """

    rec_dt = pd.to_datetime(recommendation_row["recommend_date"])

    ma5 = safe_float(recommendation_row.get("signal_ma5"))
    ma10 = safe_float(recommendation_row.get("signal_ma10"))

    if ma5 is None or ma10 is None:
        return None

    zone_high = max(ma5, ma10)
    zone_low = min(ma5, ma10)

    future_df = price_df[price_df["date"] > rec_dt].head(MAX_ENTRY_WAIT_DAYS).copy()

    if future_df.empty:
        return None

    for wait_idx, (_, bar) in enumerate(future_df.iterrows(), start=1):
        low = float(bar["min"])
        high = float(bar["max"])
        close = float(bar["close"])

        touched_zone = low <= zone_high and high >= zone_low
        held_zone = close >= zone_low

        if touched_zone and held_zone:
            return {
                "entry_mode": "PULLBACK_MA",
                "entry_date": bar["date"],
                "entry_price": round(zone_high, 2),
                "entry_reason": f"回測 MA 區間不破：{round(zone_low, 2)} ~ {round(zone_high, 2)}",
                "entry_wait_days": wait_idx,
                "pullback_zone_low": round(zone_low, 2),
                "pullback_zone_high": round(zone_high, 2),
            }

    return None


def find_entry_break_prev_high(recommendation_row: pd.Series, price_df: pd.DataFrame) -> dict | None:
    """
    突破推薦日高點才進場。

    規則：
    - 使用推薦當天 tech high 作突破價
    - 推薦後最多等 MAX_ENTRY_WAIT_DAYS 個交易日
    - 若當日最高價 >= 推薦日高點，視為觸發
    - 進場價採推薦日高點
    """
    ...
    signal_high = safe_float(recommendation_row.get("signal_high"))
    ...
    for wait_idx, (_, bar) in enumerate(future_df.iterrows(), start=1):
        ...
        if high >= signal_high:
            return {
                "entry_mode": "BREAK_PREV_HIGH",
                "entry_date": bar["date"],
                "entry_price": round(signal_high, 2),
                "entry_reason": f"突破推薦日高點：{round(signal_high, 2)}",
                "entry_wait_days": wait_idx,
                "break_price": round(signal_high, 2),
            }

    return None


def find_entry_by_mode(
    ...
    if entry_mode == "OPEN_NEXT_DAY":
        return find_entry_open_next_day(recommendation_row, price_df)

    if entry_mode == "PULLBACK_MA":
        return find_entry_pullback_ma(recommendation_row, price_df)

    if entry_mode == "BREAK_PREV_HIGH":
        return find_entry_break_prev_high(recommendation_row, price_df)
...
def simulate_exit_after_entry(
    ...
    entry_date = pd.to_datetime(entry_info["entry_date"])
    entry_price = float(entry_info["entry_price"])

    # 進場當天也納入停損 / 停利檢查
    holding_df = price_df[price_df["date"] >= entry_date].head(MAX_HOLDING_DAYS).copy()
    ...
    result = {
        ...
        "entry_reason": entry_info.get("entry_reason"),
        "entry_wait_days": entry_info.get("entry_wait_days"),
    ...
    for col in optional_entry_cols:
        if col in entry_info:
            result[col] = entry_info[col]
...
@safe_execute
def run_stock_recommendation_backtest_v2() -> pd.DataFrame | None:
    ...
    price_start = (
        pd.to_datetime(START_DATE) - timedelta(days=20)
    ).strftime("%Y-%m-%d")

    price_end = (
        pd.to_datetime(END_DATE) + timedelta(days=40)
    ).strftime("%Y-%m-%d")
    ...
    for _, row in recommendations.iterrows():
        ...
        for entry_mode in ENTRY_MODES:
            ...
            trade = simulate_exit_after_entry(
                recommendation_row=row,
                price_df=price_df,
    ...
    print(f"✅ 個股推薦回測 V2 完成：{len(result)} 筆")
    print(f"✅ 明細輸出：{BACKTEST_OUTPUT_FILE}")

    if skipped:
        skipped_df = pd.DataFrame(skipped)
        skipped_file = PROJECT_DIR / "stock_recommendation_backtest_skipped_v2.csv"
        skipped_df.to_csv(skipped_file, index=False, encoding="utf-8-sig")
        print(f"⚪ 未觸發進場紀錄輸出：{skipped_file}")
...
def summarize_by_group(df: pd.DataFrame, group_cols: list[str]) -> pd.DataFrame:
    ...
    for keys, group in df.groupby(group_cols):
        ...
        summary = summarize_result(group)
...
def print_summary(df: pd.DataFrame):
    ...
    by_mode = summarize_by_group(df, ["entry_mode"])
    by_grade = summarize_by_group(df, ["grade"])
    by_mode_grade = summarize_by_group(df, ["entry_mode", "grade"])

    with pd.ExcelWriter(SUMMARY_OUTPUT_FILE.with_suffix(".xlsx")) as writer:
        summary_df.to_excel(writer, sheet_name="summary", index=False)
        by_mode.to_excel(writer, sheet_name="by_entry_mode", index=False)
        by_grade.to_excel(writer, sheet_name="by_grade", index=False)
        by_mode_grade.to_excel(writer, sheet_name="by_mode_grade", index=False)
    ...
    print(f"✅ CSV 統計輸出：{SUMMARY_OUTPUT_FILE}")
    print(f"✅ Excel 統計輸出：{SUMMARY_OUTPUT_FILE.with_suffix('.xlsx')}")
    ...
    print(by_mode[show_cols].to_string(index=False))
    ...
    print(by_grade[show_cols].to_string(index=False))
    ...
    print(by_mode_grade[show_cols].to_string(index=False))
...
if __name__ == "__main__":
    print("🧪 ATOS Stock Recommendation Backtest V2 Started")
    ...
    print(f"進場等待：最多 {MAX_ENTRY_WAIT_DAYS} 個交易日")
    print(f"持有期間：最多 {MAX_HOLDING_DAYS} 個交易日")
    print(f"停損：{STOP_LOSS_PCT}%")
    print(f"停利：{TAKE_PROFIT_PCT}%")

    df_result = run_stock_recommendation_backtest_v2()
